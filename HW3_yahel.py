...
def deep_copy_dict(original_dict):
    ...
    for key, value in original_dict.items():
        if isinstance(value, dict):  # If the value is a dictionary
            copied_dict[key] = deep_copy_dict(
                value
            ...
            copied_dict[key] = (
                value  # For non-dictionary, non-list values, copy directly
            )
...
####### Part A #########
def remove_punctuation(seq: str) -> str:
    """
...
def remove_invalid_letters(seq: str) -> str:
    """
    this function remove invalid letters
    :return: str
    """
    valid_letters = "ATGCatgc"
    res = "".join([char if char in valid_letters else " " for char in seq])
    return res


def capitalize_letters(seq: str) -> str:
    """
...
def proofreading(seq: str) -> str:
    ...
    # find the index of the prefix and remove the characters before
    start_of_seq_index = seq.find(prefix)
    if start_of_seq_index == -1:
        return []
    valid_seq = seq[start_of_seq_index:]

    stop_condos = ["TAA", "TAG", "TGA"]
    seq_array = []
    # split the seq
    for i in range(0, len(valid_seq), 3):
        seq_array.append(valid_seq[i : i + 3])

    # check if valid suffix
    if seq_array[-1] not in stop_condos:
        if len(seq_array[-1]) != 3:
            seq_array.pop()
        seq_array.append(default_suffix)

    return seq_array


def transcript(func_seq: list[str]) -> list[str]:
    """
    this function translate dna to rna
    :return: func_seq
    """
    transcripted_array = [seq.replace("T", "U") for seq in func_seq]
    return transcripted_array


def translate(rna_seq: list[str]) -> list[str]:
    """
    ...
    while rna_seq and codon_translator(rna_seq[0]) != "stop":
        rna_list.append(codon_translator(rna_seq[0]))
        rna_seq.pop(0)
...
def get_sequences_data(sequence_corpus):
    ...
    sequences_data = {}
    pro_corpus = {
        key: preprocessing(sequence_corpus[key]) for key in sequence_corpus
    }  # copy to keep the dic like at the start
    for key in pro_corpus:
        sequences_data[key] = len(
            pro_corpus[key]
        )  # the value is the length of the amino acid sequences
...
def create_inverted_index(sequence_corpus):
    ...
    for (
        ...
        sequence_corpus.items()
    ):  # Going through both the value and the key at the same time
        for char in value_list:
            if (
                char not in inverted_index
            ):  # Add an empty dictionary when the rest of the letter does not exist
                inverted_index[char] = {}
            if (
                key in inverted_index[char]
            ):  # If the key exists we will add the number of its occurrences accordingly
...
def add_to_data(inverted_index, sequences_data, seq_id, seq):
    ...
    sequences_data[seq_id] = len(
        pro_seq
    )  # Adding the seq_id as a key, and the len of processed sequence
...
def preprocess_query(query):
    ...
    query_tuple = tuple(
        query.replace(",", "")
    )  # creating the tuple and deleting all the commas
...
def calculate_aaf_isf(amino_acid, seq_id, inverted_index, sequences_data):
    ...
    """
    shallow_inverted_index = deep_copy_dict(
        inverted_index
    )  # Perform a deep copy of the inverted_index dictionary to prevent modifying the original dictionary
    shallow_sequences_data = deep_copy_dict(
        sequences_data
    )  # Perform a deep copy of the sequences_data dictionary to prevent modifying the original dictionary
    if shallow_sequences_data.get(seq_id, 0) != 0:
        AAF = math.log(
            len(shallow_sequences_data)
            / len(shallow_inverted_index.get(amino_acid, {})),
            2,
        )  # calculation of AAF
        ISF = shallow_inverted_index.get(amino_acid, {}).get(
            seq_id, 0
        ) / shallow_sequences_data.get(
            seq_id, 1
        )  # calculation of ISF
        ans = AAF * ISF
        return round(ans, 3)
...
def get_scores_of_relevance_sequences(query, inverted_index, sequences_data):
    ...
    for amino_acid in query:
        if (
            amino_acid in shallow_inverted_index.keys()
        ):  # If the amino acid exists in the shallow_inverted_index dictionary
            for seq_id in shallow_inverted_index[amino_acid]:
                if (
                    seq_id not in scores_seq
                ):  # If the sequence ID is not already in the scores_seq dictionary, put 0
                    scores_seq[seq_id] = 0
                scores_seq[seq_id] = round(
                    scores_seq[seq_id]
                    + calculate_aaf_isf(
                        amino_acid, seq_id, inverted_index, shallow_sequences_data
                    ),
                    3,
                )  # Calculate the AAF-ISF score and add it to the scores_seq dictionary
    return scores_seq
...
print(
    get_scores_of_relevance_sequences(("R", "M", "S"), inverted_index, sequences_data)
)
...
def menu(sequence_corpus):
    ...
    if choice == "1":  # Insert a query
        ...
        if query_choice == "A":  # Show all relevant sequences
            print(
                get_scores_of_relevance_sequences(
                    preprocess_query(query),
                    create_inverted_index(sequence_corpus),
                    get_sequences_data(sequence_corpus),
                )
            )
            ...
            print(
                f"The most relevant sequence is {highest_score_seq_id} with a score of {highest_score}"
            )
        ...
        while True:
            ...
            if (
                seq_id not in sequence_corpus.keys()
            ...
            if int(seq_id) not in key_list:  # Check if sequence exists
                ...
                while True:
                    if amino_acid_after_preprocces not in preprocessing(
                        sequence_corpus[seq_id]
                    ):  # Check if amino acid is valid
                        print(
                            f"The amino acid {amino_acid_after_preprocces} is not in sequence_corpus."
                        )
                    else:
                        aaf_isf = calculate_aaf_isf(
                            amino_acid_after_preprocces,
                            seq_id,
                            create_inverted_index(sequence_corpus),
                            get_sequences_data(sequence_corpus),
                        )
        ...
        while True:
            if (
                seq_id not in get_sequences_data(seq_corpus).keys()
